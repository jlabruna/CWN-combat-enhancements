...
- Gives SWNR drones linked to an NPC pilot a reduced Burst/manual-modifier
  ...
  For linked player-character pilots, it uses pilot Attack Bonus plus the better
  of Dexterity or Intelligence and the better of Drive or Program. Dexterity
  and Program win ties. Both paths affect To Hit only and preserve native SWNR
  damage, ammunition, Shock, Trauma, targeting, and chat-card behavior.
- Resolves an imported CWN Content Pack character weapon's explicit Shoot/Stab
  ...
  weapon model so successive attacks retain the resolved Skill and Stat. A
  targeted SWNR 2.3.1 compatibility repair also preserves that Stat when the
  system's partial ammunition update would otherwise migrate it back to Ask.
...
- Drone pilots: `drone.system.crewMembers[0]`, with SWNR's resolved Actor at
  ...
  Character pilots use `system.ab`, `system.stats.dex.mod`,
  `system.stats.int.mod`, and actor-owned Drive/Program Skill ranks.
...
- Demon check and potential-damage rolls now expand to show their dice and
  modifiers using Foundry's normal Roll presentation.
- Module chat cards now use readable explicit text colors when CWN Interface
  Theme is disabled, while continuing to follow that theme when enabled.
...
- Fixed newly added and edited Demons not reaching the Network Journal because
  the submitted `classKey` field was checked under the obsolete `class` name.
- Demon submissions now save against the canonical node, await Journal and
  player-projection persistence, reject duplicate submission, and report
  failures without silently closing as if the save succeeded.
- The Add/Edit Demon dialog now stays within the browser viewport and scrolls
  its form body while keeping its action footer accessible.
...
- Fixed Network Console connections shifting relative to stationary node cards
  when the application canvas changed size.
- The SVG viewBox now tracks its rendered CSS-pixel dimensions so resizing
  cannot introduce aspect-ratio scaling or letterbox offsets.
...
- Fixed family-aware actor-sheet reload clicks falling through to SWNR's native
  one-use reload handler.
- Attached exact-magazine handling directly to rendered reload controls while
  preserving native SWNR behavior for all fallback cases.
