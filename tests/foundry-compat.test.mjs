...
test("manifest targets Foundry V14 and SWNR 2.3.1", async () => {
  ...
  assert.ok(
    manifest.esmodules.includes("scripts/cwn-combat-enhancements-v0.13.8.mjs"),
  );
...
test("versioned runtime entry point cache-busts the weapon compatibility graph", async () => {
  const entry = await fs.readFile(
    new URL("../scripts/cwn-combat-enhancements-v0.13.8.mjs", import.meta.url),
    "utf8",
  );
  ...
  assert.match(entry, /cwn-combat-enhancements\.mjs\?v=0\.13\.8/);
  assert.match(implementation, /npc-weapon-roll-compat\.mjs\?v=0\.13\.8/);
});
